...
# write_rds(df_all,"/data/ZHULAB/phenology/nab/nab_dat.rds")

nab_df<-read_rds("/data/ZHULAB/phenology/nab/nab_dat.rds") %>% as_tibble()
nab_df
```
...
## Clean up taxonomy
We also get classification on various levels for aggregation.
```{r taxa}
# taxa_df<-df_all %>% 
#   distinct(taxa) %>% 
...
#                            TRUE~kingdom)) %>% 
#   as_tibble()
# write_rds(taxa_df,"/data/ZHULAB/phenology/nab/nab_dat_taxa.rds")

nab_taxa_df<-read_rds("/data/ZHULAB/phenology/nab/nab_taxa.rds") %>% as_tibble()
nab_taxa_df
```
...
```{r}
cairo_pdf("./nab/output/figures/1 map_calendar.pdf", width=6, height=6)
...
df_comm_all<-nab_df %>% 
  ...
  group_by(taxa_raw, taxa_clean, Date, lat, lon, station,location, id, family,genus) %>% 
  summarize(count=sum(count)) %>% 
  ungroup() %>% 
  group_by(taxa_raw, taxa_clean, lat, lon, station, location, id, family,genus) %>% 
  complete(Date = seq(min(Date),max(Date), by="1 day"))%>%
  mutate(count_ma = roll_mean(count, n = 28, align = "right", fill = NA, na.rm = T)) %>%
  ungroup() %>% 
  drop_na(family) %>% 
  group_by(station, location, id, Date,family, lon, lat) %>% 
...
taxa = "Cladosporiaceae" # "Cladosporiaceae"
df_pheno<-df_comm_all %>%
  # filter(location %in% site_list) %>% 
  filter(family==taxa) %>% 
  mutate(year=as.integer(format(Date, "%Y"))) %>% 
  mutate(doy=as.integer(format(Date, "%j"))) %>% 
  group_by(station, location, id, lat, lon, year) %>% 
...
df_pheno_complete<-vector(mode="list",length=nrow(df_pheno_group))
for(i in 1:nrow(df_pheno_group)) {
  df_pheno_1y<-df_pheno %>% 
    ...
    mutate(station=df_pheno_group$station[i],
           location=df_pheno_group$location[i],
           id=df_pheno_group$id[i],
           lon=df_pheno_group$lon[i],
           lat=df_pheno_group$lat[i])
  if (nrow(df_pheno_1y %>% drop_na(count_agg))>=365*3/5) {
    df_pheno_complete[[i]]<-df_pheno_1y
  }
  # print(i)
}
df_pheno_complete<-rlist::list.clean(df_pheno_complete) %>% 
  bind_rows()

p_pheno<-ggplot(df_pheno_complete %>% 
         filter(location %in% site_list))+
  geom_line(aes(x=doy+60, y=count_agg, col=year, group=year), alpha=0.5)+
  scale_y_continuous(trans = log_trans(), 
                     breaks = trans_breaks("log", function(x) exp(x)),
                     labels = trans_format("log", math_format(e^.x)))+
  scale_x_continuous(labels=labelfunc_x)+
