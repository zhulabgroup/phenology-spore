...
(p_metrics <- plot_metrics(df_sample, df_analysis))
...
if (.fulldata) {
  ...
  (p_calendar <- plot_calendar(df_ts))
  ...
  if (.figsave) {
    ...
    file.copy(
      from = str_c(.path$output, "figures/Figure_2.png"),
      to = str_c("../inst/", "figures/Figure_2.png")
    )
  }
} else {
...
df_trend_station_SAS <- calc_trend_station_TS(df_in = df_analysis, metric = "SAS", pct = 0.8)
(p_trend_map_TS_SAS <- plot_trend_map_TS(df_in = df_trend_station_SAS, metric = "SAS") +
  guides(size = "none"))
