...
if (.fulldata) {
  ...
  if (.tablesave) {
    ...
    file.copy(
      from = str_c(.path$output, "tables/tbl_station_meta_data.csv"),
      to = str_c("../inst/", "tables/tbl_station_meta_data.csv")
    )
  }
} else {
...
df_trend_taxa <- summ_composition_trend(df_comm, ls_family = c("Unidentified", "Cladosporiaceae"))
