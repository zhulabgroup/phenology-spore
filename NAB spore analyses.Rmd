...
df_ts<-nab_with_taxa_df%>% 
  group_by(lat, lon, station, location, id, family, genus, taxa) %>% 
  ...
  summarize(count_agg=sum(count)) %>% 
  ungroup() %>% 
  mutate(count_agg=case_when(count_agg>=5~count_agg))
