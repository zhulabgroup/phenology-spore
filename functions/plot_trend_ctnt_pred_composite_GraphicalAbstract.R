...
source(str_c(.path$ana_fun, "plot_trend_ctnt_pred_GraphicalAbstract.R"))
...
for (m_metric in c("SOS", "SAS", "EOS", "EAS", "LOS", "LAS", "ln_Ca", "ln_Cp", "ln_AIn", "ln_ASIn")) {
  ...
  # plot
  p_trend_ctnt_pred <- plot_trend_ctnt_pred_GraphicalAbstract(df_in = df_trend_station_col, model = m_trend, metric = m_metric, pct = 0.8)
  p_trend_ctnt_pred_list <- c(p_trend_ctnt_pred_list, list(p_trend_ctnt_pred))
}

title1 <- ggdraw() + draw_label(" ", hjust = 0.5, vjust = 0.5, fontface = "bold")
title2 <- ggdraw() + draw_label(" ", hjust = 0.5, vjust = 0.5, fontface = "bold")
...
ggsave(
  paste0(.path$out_fig, "STOTEN/Figure_GA_trends.png"),
  plot = p_trend_GA,
