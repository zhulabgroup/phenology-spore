...
p_load(tidyverse)
p_load(raster)
p_load(rgdal) # install.packages("rgdal", configure.args = "--host=host")
p_load(gdalUtils)
p_load(stringr)
p_load(httr)
p_load(sp)
...
# p_load(gstat) # can't load for some reason
p_load(R.utils)
p_load(rsconnect)
p_load(Rcpp)
p_load(lubridate)
...
compare_stats<-function ( obs_ori, pred_ori,obs=NULL,pred=NULL, range=NULL) {
  if (length(obs_ori)>0 & length(pred_ori)>0) {
...
GeoDistanceInMetresMatrix <- function(df.geopoints){
  ...
  GeoDistanceInMetres <- function(g1, g2){
    ...
    DistM <- function(g1, g2){
      require("Imap")
      return(ifelse(g1$index > g2$index, 0, Imap::gdist(lat.1=g1$lat, lon.1=g1$lon, lat.2=g2$lat, lon.2=g2$lon, units="m")))
    }
