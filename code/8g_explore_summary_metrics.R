...
p_antt<- ggplot() +
  ...
  scale_x_datetime(
    ...
    date_labels = c("1", "sos", "sas", "peak date", "eas", "eos", "365")
    ) +
  coord_cartesian(
  ...
  scale_fill_manual(
    name='',
    values=c("annual integral" = "orange", "allergy season integral" = "dark red"),
  ...
  labs(title = expression(paste(bold("A      Metrics of Fungal Spore Phenology")))) +
  theme(
...
p_metrics_space <- ggplot() +
  theme_void() +
  theme(plot.margin = margin(10, 10, 10, 10))
p_metrics_1 <- plot_grid(p_metrics_b, p_metrics_space,
                         ncol = 1,
                         rel_heights = c(1, 1))
p_metrics_2 <- plot_grid(p_antt, p_metrics_c, ncol = 1,
                         rel_heights = c(1, 1))
