...
# plot to compare results of three smoothing methods for each station
...
for (i in c(1:55)) {
  p <- plot_smooth_compare_station(df_smooth, i)
  print(p)
}
dev.off()
```

# plot to compare results of three smoothing methods for all stations
```{r plot smoothing compare all}
source("plot_smooth_compare.R")
gg_smooth_compare_all <- plot_smooth_compare_all(df_smooth, 8)
pdf(
  "~/Github/spore_phenology/output/figures/smooth_compare/p_compare_1800.pdf",
  width = 8*8,
  height = 8*.618*7
)
print(gg_smooth_compare_all)
dev.off()
```

# manipulate lambda, compare two whittaker methods for df_full
```{r}
source("tidy_smoothfillwhit_station.R")
source("tidy_smoothweiwhit_station.R")
source("plot_smooth_compare.R")
df_wavelet <- df_smooth %>% 
  dplyr::select(lat, lon, station, state, country, id, date, count_wavelet) %>% 
  right_join(df_full, by = c("lat", "lon", "station", "state", "country", "id", "date"))
df_plot <- tibble(
  n = 1:30,
  a = vector("list", 30)
)
for (i in c(1:30)) {
  ...
  df_smooth_lmd <- df_wavelet %>%
    ...
    left_join(df_offset, by = c("lat", "lon", "station", "state", "country", "id")) # combine offset for visualization
  gg_smooth_compare_all <- plot_smooth_compare_all(df_smooth_lmd, 8, wavelet = T)
  df_plot$a[[i]] <- gg_smooth_compare_all
}
pdf(
  "~/Github/spore_phenology/output/figures/smooth_compare/p_compare_smooth_three.pdf",
  width = 8*8,
  height = 8*.618*7
  )
for (i in c(1:30)) {
