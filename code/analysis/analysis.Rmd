...
source("~/Github/spore_phenology/code/analysis/filt_taxa_station_raw_data.R")
df_siteyear <- filt_taxa_station_raw_data(df_spore, "Total", 10, 3)
```

# rename stations and tag number for each station
```{r message=FALSE, warning=FALSE}
source("~/Github/spore_phenology/code/analysis/tidy_rename_number_station.R")
...
source("~/Github/spore_phenology/code/analysis/tidy_fulldate_station.R")
...
source("~/Github/spore_phenology/code/analysis/read_offset_station.R")
df_full <- df_full %>%
...
<!-- # summarize missing data percentage -->
<!-- ```{r} -->
<!-- source("~/Github/spore_phenology/code/analysis/summ_miss_stationyear_raw_data.R") -->
<!-- df_miss <- df_full %>% summ_miss_stationyear_raw_data(column_name = count) -->
<!-- ``` -->
...
<!-- ```{r download daymet} -->
<!-- df_meta = read_rds(str_c(.path$dat_process, "2023-04-25/meta.rds")) -->
<!-- source("~/Github/spore_phenology/code/analysis/down_daymet_station_date.R") -->
...
<!-- ```{r} -->
<!-- source("~/Github/spore_phenology/code/analysis/calc_climate_stationspyr.R") -->
...
# write_rds(df_ana_full, str_c(.path$dat_process, "2023-04-25/dat_ana_full_as2000.rds"))
# df_ana_short <- right_join(df_daymet_annual, df_metrics, by = c("lat", "lon", "station", "city", "state", "country", "id", "n", "offset", "year_new"))
# write_rds(df_ana_short, str_c(.path$dat_process, "2023-04-25/dat_ana_short_as2000.rds"))
```

# filter stations for each metric based on the number of years
```{r}
source("~/Github/spore_phenology/code/analysis/filt_station_NofYears_metric.R")
