...
for (i in 1:length(file_list)) {
  ...
  if (min(start_n)==1) {
    ...
    station<-meta_dat[2] %>% strsplit(split=": ") %>% unlist() %>% tail(1)
  }

  location<-file %>% strsplit(split=c("/")) %>%unlist() %>%  tail(1) %>% strsplit(split=" \\(") %>% unlist() %>% head(1)
  ...
  if (length(start_n)==1) { # only pollen, no spores
    ...
    if (is.na(pollen_dat[2,1])) { # meaning finer taxonomic resolutions available)
      genus_names <- read_excel(
        ...
        mutate(new=case_when(is.na(new)~old,
                             TRUE~new))
      pollen_dat <- read_excel(
        file,
        skip = start_n[1]-1,
    ...
    pollen_dat <- read_excel(
      ...
      col_types = c("date",
                    rep("numeric", ncol(read_excel(
                      file,
                      skip = start_n[1]-1,
                      n_max = (start_n[2]-2)-(start_n[1]+1)))-1))
    )
...
write_rds(df_all,str_c(.path$nab_clean,"nab_dat.rds"))
read_rds('/nfs/turbo/seas-zhukai/phenology/nab/nab_dat_taxa.rds')
