...
# each station
ggplot(data = df_smooth %>% filter(id == 50)) +
  geom_line(aes(x = date, y = count_smooth)) +
  scale_y_continuous(
    trans = scales::log_trans(),
    breaks = scales::trans_breaks("log", function(x) exp(x)),
  ...
  ylab("count") +
  geom_vline(xintercept = (df_metrics %>% filter(id == 50) %>% mutate(peak_date = as.POSIXct(as.Date(peak_doy, origin = paste0(year, "-01-01"))) -1))$peak_date, color = "gray")
# select data
