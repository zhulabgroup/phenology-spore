hist_mean<-ts %>% 
  dplyr::select(site_id, date, pollen) %>% 
  filter(date<middate) %>% 
  mutate(doy=as.integer(format(date, "%j"))) %>% 
  group_by(site_id, doy) %>% 
  summarise(climatology_mean=mean(pollen, na.rm=T),
            climatology_sd=sd(pollen, na.rm = T)) %>% 
  ungroup()

climatology<-ts %>% 
  dplyr::select(site_id, date) %>% 
  full_join(expand_grid(site_id=unique(ts$site_id), date=date_list),
            by=c("site_id", "date")) %>% 
  mutate(doy=as.integer(format(date, "%j"))) %>% 
  left_join(hist_mean,
            by=c("site_id", "doy")) %>% 
...
for (j in 1:length(date_list)) { #date
  for (v in 1:length(var_list)) { #covariate
    if (var_list[v] %in% c("pollen", "tmean", "prcp", "vp")) {
    ...
    if (var_list[v] %in% c("climatology")) {
      ts_date<-climatology %>% filter(date==date_list[j])
      if (nrow(ts_date)>0) {
        x[,j,v]<-ts_date["climatology_mean"] %>% unlist()%>% as.numeric()
      } else {
        x[,j,v]<-rep(NA, nrow(coord_df))
      }
    }
