  ...
  Rprop_res <-
    foreach(
      ...
      lpost <- function(p,sample_n) {
        ...
        return(GPSDM(pars = p, distMat = distMat, basisX = X_basis, basisP = P_basis,basisD = D_basis, basisY = Y_basis, newX = X_train[minibatch, , drop = F], newP = P_train[minibatch, , drop = F], newD = D_train[minibatch, , drop = F], newY = Y_train[minibatch, , drop = F], priors_tr = list(E = pars_prev[, i, drop = F], V = pars_var_prev[[i]]), mode = c("optimize")))
      }
      print("start")
...
phi_weighted_mean<-C_df %>% 
  ...
  summarise (phi=stats::weighted.mean(phi, particle_wts)) %>% 
