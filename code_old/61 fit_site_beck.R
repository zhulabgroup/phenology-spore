...
Code <- nimbleCode({
  ...
  # mu_mn ~ dnorm(mean=4, var=10)
  # mu_mx ~ dnorm(mean=7, var=10)
  # constraint_data1 ~ dconstraint( mu_mx > mu_mn )
  # mu_sos ~ dunif(min=0, max=365)
  # mu_eos ~ dunif(min=0, max=365)
  # constraint_data2 ~ dconstraint( mu_eos > mu_sos )
  # mu_rsp ~T(dnorm(mean=0.05, var=0.1), 0, )
  # mu_rau ~ T(dnorm(mean=0.05, var=0.1), 0, )
  ...
  for (i in 1:N) {
    ...
    y[i] ~dnorm(mn[i] + (mx[i] - mn[i]) * (1/(1+exp(-rsp[i]*(doy[i]-sos[i])))+1/(1+exp(rau[i]*(doy[i]-eos[i])))),sigma)
  }
})
...
dl<-function (doy
  ...
  y = mn + (mx - mn) * (1/(1+exp(-rsp*(doy-sos)))+1/(1+exp(rau*(doy-eos))))
...
get_betas<-function(MCMC) {
  ...
  df<-bind_rows(beta1_mn, beta1_mx, beta1_sos, beta1_eos,beta1_rsp, beta1_rau,
    ...
    mutate(param=factor(param, levels=c("mn","mx", "sos", "eos", "rsp", "rau")))
