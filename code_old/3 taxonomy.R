...
flower<-read_rds("./nab/flower_raw_data.rds")
...
taxa_df_nab<-df_all %>% 
  ...
  mutate(taxa_clean=case_when(taxa_clean=="Rusts"~"Pucciniales",
...
taxa_df_inat<-flower %>% 
  dplyr::select(taxa_clean=scientific_name) %>% 
  distinct() %>% 
  mutate(dataset="inat")

taxa_df<-bind_rows(taxa_df_nab,taxa_df_inat)

gnr_datasources()
# check if all taxa names are valid
resolve_df<-taxa_df%>% 
  pull(taxa_clean) %>% 
...
taxa_id_df<-resolve_df %>% 
  ...
  mutate(data_source=case_when(data_source_id==4~"ncbi",
...
write_csv(taxa_df, "./nab/nab_and_inat_taxa.csv")

intersect(taxa_df %>% filter(dataset=="nab") %>% pull(genus) %>% unique() ,
          taxa_df %>% filter(dataset=="inat") %>% pull(genus) %>% unique())

intersect(taxa_df %>% filter(dataset=="nab") %>% pull(family) %>% unique() ,
